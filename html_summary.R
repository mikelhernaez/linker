...
create_html_summary <- function(rfiles=c("data/Tumor_OV50.tar8855_reg638.VBSR.m100_b10.rds",
  ...
  show(paste0("Loading id conversion table: ", runinfo$mapfile, "..."))
  iso_table <- as.matrix(read.table(runinfo$mapfile, header = T, sep = "\t",
                                    quote = ""))
  ...
  htmlinfo <- create_index_page(outdir = runinfo$outdir,
  ...
  if (file.exists(runinfo$evidfile)){
    show(paste0("Loading chip evidence table: ", runinfo$evidfile, "..."))
    weighted_chip_evidence <- as.matrix(read.table(runinfo$evidfile, header = T,
                                                   row.names = 1, sep = "\t",
                                                   quote = ""))
    show("chip evidence regulators: ")
    ...
    show(signif(binary_chip_summary / ( dim(weighted_chip_evidence)[1] *
  ...
  for (rfile in runinfo$rfiles){
    ...
    for ( mymodmeth in mymodmeths){
      ...
      for ( graphmeth in graphslist){
        ...
        if (runtypestr == "single_gene"){
          show(paste0("****************Single Gene Network ", graphmeth))
          rungraphs <- list(rundata$graphs[[graphmeth]])
        } else {
          show(paste0("****************", mymodmeth, " Method: ", graphmeth))
          rungraphs <- rundata$graphs[[mymodmeth]][[graphmeth]]
        }
        final_summary <- summarize_rungraphs(rungraphs = rungraphs,
                                             iso_table = iso_table,
        ...
        write.table(allsummaries, paste0(htmlinfo$htmldir, resultspath),
