...
mod_graphs<-list()
mod_graphs$VBSR_VBSR<-LINKER_compute_modules_graph(enriched_modules_vbsr_100, lognorm_est_counts, mode="VBSR")
mod_graphs$VBSR_LASSOmin<-LINKER_compute_modules_graph(enriched_modules_vbsr_100, lognorm_est_counts, mode="LASSOmin")
mod_graphs$VBSR_LASSO1se<-LINKER_compute_modules_graph(enriched_modules_vbsr_100, lognorm_est_counts, mode="LASSO1se")
mod_graphs$VBSR_LM<-LINKER_compute_modules_graph(enriched_modules_vbsr_100, lognorm_est_counts, mode="LM")

mod_graphs$LASSO_VBSR<-LINKER_compute_modules_graph(enriched_modules_lasso_100, lognorm_est_counts, mode="VBSR")
...
all_graphs$LASSOmin<-LINKER_compute_graph_all_LASSO_min(lognorm_est_counts, lincs_filtered_idx, protein_filtered_idx)
all_graphs$LASSO1se<-LINKER_compute_graph_all_LASSO_1se(lognorm_est_counts, lincs_filtered_idx, protein_filtered_idx)
all_graphs$LM<-LINKER_compute_graph_all_LM(lognorm_est_counts, lincs_filtered_idx, protein_filtered_idx)

all_GEAs<-LINKER_compute_graph_enrichment_geneSets_graph_list(pathway_genes,all_graphs)
